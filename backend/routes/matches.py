...
async def _get_active_resume(db, user_id: str) -> Optional[dict]:
...
async def _get_resume_text(db, resume_id: str, user_id: str) -> str:
...
async def _compute(db, user: dict, resume: dict, opp: dict, *, with_tfidf: bool) -> MatchResult:
    ...
    resume_skills = list({*(parsed.get("skills") or []), *(parsed.get("ai_skills") or []), *(profile.get("skills") or [])})
